defmodule SymphonyElixir.Config.Schema do
  ...
  defmodule Claude do
    ...
    embedded_schema do
      ...
      field(:model, :string, default: "claude-opus-4-8")
    ...
    def changeset(schema, attrs) do
      schema
      |> cast(attrs, [:command, :model, :effort, :permission_mode, :env_file, :turn_timeout_ms], empty_values: [])
      |> validate_required([:command, :model, :effort, :permission_mode])
      |> validate_inclusion(:effort, ["low", "medium", "high", "xhigh", "max"])
      |> validate_number(:turn_timeout_ms, greater_than: 0)
    end
  end
  ...
  defp tracker_api_key_fallback("kaneo"), do: System.get_env("KANEO_API_KEY")
